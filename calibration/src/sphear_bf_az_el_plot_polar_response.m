...
%% *SpHEAR Project
%%
%% sphear_bf_az_el_plot_polar_response: plot polar response of B format components
%%
%% usage:
%%   sphear_bf_plot_polar_response(B, M, AZ, EL, frefmin, frefmax, fmin, fmax, Noct, Nfft, Fs, band, cardioid = 0, azim = 45)
%%
%% B: B format signal
%% M: projections matrix
%% AZ: azimuth angles
%% EL: elevation angles
...
function sphear_bf_az_el_plot_polar_response(B, M, AZ, EL, frefmin, frefmax, fmin, fmax, Noct, Nfft, Fs, band, cardioid = 0, azim = 45, subplots = 1)
  ...
  for ib = 1:numel(band)
	...
	sphear_bf_az_el_plot(BP, M, AZ, EL, fe(i), fe(i + 1), 1.25, cardioid, subplots);
      end
      idx = idx + 1;
    end
  end
