...
%% *SpHEAR Project
%%
%% sphear_af_octa_calibrate: read reference microphone signal, filter with
%%                           convolution kernel created by DRC and filter
%%                           all recorded signals, calibrated A format files
%%                           are written to disk
%%
%% usage:
%%   [] = sphear_af_octa_calibrate(measurements, keep, fade)
%%
...
function [] = sphear_octa_af_calibrate(measurements, keep, fade, suffix);
  if (! exist("suffix"))
    suffix = "";
  end
  ...
  octanames = cellstr(['1'; '2'; '3'; '4'; '5'; '6'; '7'; '8']);
  loc_min = intmax;
  loc_max = 0;
  %% all requested measurements
  for m = measurements
    printf("processing measurement %d\n", m);
    %% read in capsule signals and normalize them
    for c = 1:8
      sig = wavread(["../data/current/af/a-format-" sprintf("%s%03d", suffix, m) "-" char(octanames(c)) ".wav"]);
  ...
  for m = measurements
    for c = 1:8
      wavwrite(cal_sig(m,c,:)(range) .* wend, Fs, ["../data/current/af/a-format-cal-" sprintf("%s%03d", suffix, m) "-" char(octanames(c)) ".wav"]);
    end
    %% wavwrite(cal_sig, Fs, ["../data/current/af/a-format-cal-" sprintf("%s%03d", suffix, m) ".wav"]);
  end
end
