...
%%
%% usage:
%%   [F FE A2BF] = sphear_a2b_all(m, fmin, fmax, Noct, Fs, Nsamp, Npre, plt)
%%
...
function [B] = sphear_a2b_fons_all(fmin, fmax, Noct, Nfft, Fs, plt)
  ...
  ref = sphear_bf_power_fons(600, 1200, Nfft, Fs, 1);
  norm = mean(ref(:,1));

  for i = (1:size(fc))
    B(:,:,i) = sphear_bf_power_fons(fe(i), fe(i + 1), Nfft, Fs, plotz) / norm;
    if (plt == 1)
      sphear_bf_plot(B(:,:,i), fe(i), fe(i + 1), 3.5);
    end
  end
