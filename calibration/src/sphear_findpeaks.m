...
function [peakamps,peaklocs,peakwidths,resid] = ...
	 sphear_findpeaks(data,npeaks,minwidth,maxwidth,minpeak,debug);
  len = length(data);
  if nargin<6,
    debug = 0;
  end;
  ...
  ipeak=0; 
  while ipeak<npeaks
    [ploc, pamp, pcurv] = sphear_maxr(data);
	...
	ttl = sprintf('Peak %d = %0.2f at %0.2f, width %d',ipeak,pamp,ploc,pwid);
	if x == -1
          pdebug = 0;
	end
      end
  ...
  if (ipeak<npeaks),
    warning(sprintf('*** peaks.m: only %d peaks found instead of %d',ipeak,npeaks));
